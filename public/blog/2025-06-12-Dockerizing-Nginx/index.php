...
DC;
  ...
  <p>I changed the original <code>cloudflare-tunnel-net</code> network to no longer be external so that it's only responsible for connecting the Cloudflare tunnel and Nginx. Instead, now I have the external <code>nginx-proxy-net</code> network which is responsible for facilitating the reverse proxies to website containers elsewhere (I will be changing their setups to use this network).</p>

  <p>Now in theory, the flow of HTTP requests through the tunnel could access the appropriate container, since I mapped the domain name in the Cloudflare dashboard to http://nginx which would be new the service URL in the tunnel.</p>
